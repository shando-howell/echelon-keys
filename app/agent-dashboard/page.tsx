...
export default function AgentDashboard() {
    ...
    const createTestProperty = useMutation(api.properties.createProperty);

    const handleFireTestMutation = async () => {
        try {
            console.log("Firing secure mutation...");
            const newPropertyId = await createTestProperty({
                title: "Test Auth Villa",
                description: "Checking the Clerk-Convex pipeline.",
                price: 15000000,
                propertyType: "house",
                status: "active",
                bedrooms: 4,
            ...
            console.log("SUCCESS! Property created with ID:", newPropertyId);
            alert(`Success! Check the Convex dashboard. ID: ${newPropertyId}`);
        } catch (error) {
            console.error("Mutation failed:", error);
            alert("Mutation blocked! Check the console.");
        }
    }
        ...
        <div className="min-h-screen bg-gray-50">
            ...
            <main className="p-8 max-w-7xl mx-auto">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    {/* Testing Auth */}
                    <div 
                        onClick={handleFireTestMutation}
                    >
                        <h3>Test Auth Pipeline</h3>
                    </div>


                    {/* Action Card: Add New Listing */}
                    <div className="bg-white p-6 rounded-xl border shadow-sm flex flex-col items-center
                    justify-center text-center hover:shadow-md transition-shadow cursor-pointer">
